...
fun SpecialGameModesScreen(onModeSelected: (String) -> Unit, onBack: () -> Unit) {
    ...
    val creamsTier = remember { ProgressionManager.getProgressionTier(context, "Creams & Emulsions") }
    val solidsTier = remember { ProgressionManager.getProgressionTier(context, "Solid dosage forms") }
    val preservativesTier = remember { ProgressionManager.getProgressionTier(context, "Preservatives & antioxidants") }
        ...
        Column(
            ...
            LazyColumn(modifier = Modifier.weight(1f)) {
                items(specialGameModes) { mode ->
                    val isLocked = when (mode.id) {
                        "emulsion_types", "lanette_lingering" -> creamsTier != ProgressionTier.FULLY_UNLOCKED
                        "cellulose_connoisseur" -> solidsTier != ProgressionTier.FULLY_UNLOCKED
                        "stunning_stability" -> preservativesTier != ProgressionTier.FULLY_UNLOCKED
                        else -> false
                    }
