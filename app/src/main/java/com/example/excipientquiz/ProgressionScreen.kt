...
fun ProgressionScreen(onBack: () -> Unit) {
    ...
    val progressionCategories = quizModes.keys.filter { it != "All Excipients" && it != "Other" }.sorted()
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            items(progressionCategories) { category ->
                ProgressionItem(category = category)
...
@Composable
private fun ProgressionItem(category: String) {
    val context = LocalContext.current
    val tier = ProgressionManager.getProgressionTier(context, category)
